...
This is `lycet-fork`, forked from `giansalex/lycet` specifically to fix two production bugs found while integration-testing against real SUNAT. Load the `lycet-fork` skill (`.claude/skills/lycet-fork/`) before touching `DespatchController`, `config/serializer/Despatch.*.yml`, or `composer.json`'s `greenter/*` pins — it documents exactly why the two bugs happen, current fix status, and how to verify fixes against real SUNAT.
...
Docker: `docker build -t lycet .` then run with `data/` volume-mounted (cert.pem, logo.png, optional `empresas.json`). CI (`.github/workflows/symfony.yml`) runs `php bin/phpunit` on PHP 7.4 against every push/PR to `master`.
